...
**Optional polish — PARTIALLY DONE (2026-08-08):** the highest-risk current-value mentions (gold, WTI, US 10Y, DXY) in `snapshot[]`, `techMacro[]`, `techCrypto[]`, and the XLE thesis are now wrapped in `<span data-mref="KEY">` and synced from `macro.json` by `syncMacroProse()` (runs at the end of `renderMacro()`), so a single macro.json edit updates every mention at once. Verified by temporarily mutating `macro.json`'s wti value and confirming all 3 wti spans updated together. **Remaining:** biRate/usdidr/fedFunds/usCPI/usPCE mentions in prose (bottomLine, position thesis, news) are still plain hard-typed text — not yet wrapped, because those numbers are usually qualitative ("firmer", "hike working") rather than a single hard number, so the risk of divergence is lower. Support/resistance technical levels (e.g. "Support ~$74; resistance ~$82") are intentionally left as plain text since they're not current values. To extend: wrap any new numeric current-value mention in `<span data-mref="macroKey">` using the same macro.json key.
